...
export const SettingsPanel: React.FC<SettingsPanelProps> = ({
  tc, activeEngineId, youtubeApiKey, setYoutubeApiKey, isAiMode, geminiApiKey, setGeminiApiKey,
  geminiModel, setGeminiModel, isMonochromeMode, setIsMonochromeMode, workspaceSearchFallback,
  setWorkspaceSearchFallback, setShowSettingsPanel
}) => {
  return (
    <motion.div initial={{ opacity: 0, y: 10 }} animate={{ opacity: 1, y: 0 }} exit={{ opacity: 0, y: 10 }} className="mt-6 w-full">
      <div className={`rounded-xl p-4 backdrop-blur-md space-y-4 ${tc.settingsPanelBg} ${tc.settingsPanelBorder}`}>
        {activeEngineId === 'youtube' && (
          <div>
            <label className={`block text-[10px] font-semibold mb-2.5 uppercase ${tc.settingsLabelText}`}>YouTube API Key</label>
            <input
              type="password"
              value={youtubeApiKey}
        ...
        {isAiMode && (
          <>
            <div>
              <label className={`block text-[10px] font-semibold mb-2.5 uppercase ${tc.settingsLabelText}`}>Gemini API Key</label>
              <input
                type="password"
                value={geminiApiKey}
            ...
            <div>
              <label className={`block text-[10px] font-semibold mb-2.5 ${tc.settingsLabelText}`}>Modelo</label>
              <input
                type="text"
                value={geminiModel}
        ...
        <div className={`flex items-center justify-between ${activeEngineId === 'youtube' ? `pt-2 border-t ${tc.settingsDivider}` : ''}`}>
          <label className={`text-[10px] font-semibold ${tc.settingsLabelText}`}>Modo Monocromático</label>
          <button
        ...
        {activeEngineId === 'workspace' && (
          <div className={`flex items-center justify-between pt-2 border-t ${tc.settingsDivider}`}>
            <label className={`text-[10px] font-semibold ${tc.settingsLabelText}`}>Funcionar como Busca</label>
            <button
        ...
        <button onClick={() => setShowSettingsPanel(false)} className={`w-full py-2 rounded-lg text-[11px] font-medium transition-colors ${tc.settingsButtonBg} ${tc.settingsButtonHoverBg}`}>
